...
export default function AccountMenu() {
  ...
  const item = (Icon: typeof UserRound, label: string, href: string, danger?: boolean) => (
    <Link href={href} role="menuitem" onClick={() => setOpen(false)} style={{ ...itemStyle, color: danger ? "var(--danger-fg)" : "var(--fg-1)" }}
      onMouseEnter={(e) => { e.currentTarget.style.background = danger ? "var(--danger-fill)" : "var(--cream-tert)"; }}
      onMouseLeave={(e) => { e.currentTarget.style.background = "transparent"; }}>
      <Icon size={17} color={danger ? "var(--danger-fg)" : "var(--fg-2)"} /><span style={{ flex: 1 }}>{label}</span>
    </Link>
  );

  return (
    <div ref={wrap} style={{ position: "relative" }}>
      <button type="button" aria-haspopup="menu" aria-expanded={open} aria-label="Tu cuenta" onClick={() => setOpen((o) => !o)}
        style={{ display: "flex", alignItems: "center", gap: 7, padding: "3px 6px 3px 3px", border: "1px solid " + (open ? "var(--outline-variant)" : "transparent"), borderRadius: 999, background: open ? "var(--surface)" : "transparent", cursor: "pointer" }}>
        <span style={avatarStyle}>{initials}</span>
        <ChevronDown size={15} color="var(--fg-3)" style={{ transform: open ? "rotate(180deg)" : "none", transition: "transform .16s ease-out" }} />
      </button>

      {open && (
        <div className="pop" role="menu" style={{ position: "absolute", top: "calc(100% + 10px)", right: 0, width: 264, background: "var(--surface)", border: "1px solid var(--outline-variant)", borderRadius: 14, boxShadow: "var(--shadow-pop)", overflow: "hidden", zIndex: 60 }}>
